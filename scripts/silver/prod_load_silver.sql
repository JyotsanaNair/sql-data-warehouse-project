...
BEGIN
	...
	CASE WHEN UPPER(TRIM(cst_marital_status)) = 'S' then 'Single'
		 ...
		 ELSE 'n/a'
	END AS cst_marital_status, -- Normalise marital status values into readable format
	CASE WHEN UPPER(TRIM(cst_gndr)) = 'F' then 'Female'
		 WHEN UPPER(TRIM(cst_gndr)) = 'M' then 'Male'
		 ELSE 'n/a'
	END AS cst_gndr, -- Normalise gender values into readable format
	cst_create_date
	FROM (
		SELECT 
		*, 
	...
	SELECT 
		...
		CASE WHEN sls_price IS NULL OR sls_price <= 0
			...
			ELSE sls_price -- Derive price is original value is incorrect
		END AS sls_price
